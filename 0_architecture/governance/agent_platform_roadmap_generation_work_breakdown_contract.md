...
| `P18.9.2` | `ticket_type` | `implementation` |
| `P18.9.2` | `objective` | Implement the Pepper Control Center Overview at the existing `/agent-platform/overview` route by composing current governed state, next action, human-attention, execution, and governed-state summaries from existing Pepper/Hermes read models while preserving the accepted P18.9.1 shell, routing, navigation, plugin, and extension architecture. |
| `P18.9.2` | `context` | P18.9.1 accepted the compact Pepper shell, protected `/agent-platform/*` namespace, CONTROL group, route ownership, and extension/plugin collision boundaries.<br>The existing product-owned Overview descriptor is `RUNTIME_OVERVIEW_DESCRIPTOR` at `/agent-platform/overview` with navigation label `Overview`.<br>The existing runtime overview source already parses bounded status and `agent_platform_workflow_control` fields through `parseRuntimeOverviewSnapshot` and `RuntimeWorkflowControl`.<br>Overview must answer: What am I working on?<br>Overview must answer: What state is it in?<br>Overview must answer: Does anything require human attention?<br>Overview must answer: Is Pepper executing anything now?<br>Overview must answer: What is the next governed action? |
| `P18.9.2` | `predecessor_evidence` | P18.9.1 shell/routing/navigation handoff accepted CONTROL as `Overview, Lead Agent` and preserved the six Pepper primary groups: CONTROL, WORK, AGENTS, AUTOMATION, RESOURCES, SYSTEM.<br>P18.9.1 preserved existing shell/navigation/extension primitives instead of adding a second router, sidebar, plugin loader, route registry, or navigation runtime.<br>P18.9.1 kept contextual/detail routes out of primary navigation and preserved backend/API authority boundaries. |
| `P18.9.2` | `dependency_context` | Roadmap dependency `P18.9.1` must be accepted before P18.9.2 execution; this remains roadmap metadata and must not create a compile-only dependency-plan edge in the single-ticket generation packet.<br>P18.9.3 Lead Agent Product Experience, P18.9.4 Work: Projects and Tickets Workspace, P18.9.5 Work: Approvals Decision Experience, and P18.9.6 Work: Execution Lifecycle, Review, Recovery, and Git Handoff UX remain downstream specialization tickets and must not be implemented in the Overview ticket. |
| `P18.9.2` | `information_architecture` | CONTROL: Overview, Lead Agent.<br>WORK: Projects, Approvals, Executions.<br>AGENTS: Agents.<br>AUTOMATION: Automation, Integrations.<br>RESOURCES: Resources.<br>SYSTEM: Settings.<br>Overview remains before Lead Agent in CONTROL. |
| `P18.9.2` | `required_surfaces` | Current Work summary for current project, macroproject, current/generated ticket, workflow status, review state, validation state, recovery state, and human Git handoff state.<br>Next Governed Action summary sourced from workflow control next action.<br>Needs Attention summary sourced from pending approvals, pending ticket approval count, blocked/recovery state, and unavailable read-model states.<br>Execution summary for idle execution and active execution status using existing execution read models.<br>Governed State summary for workflow status, queue state, approval state, review state, validation state, recovery state, and human Git handoff state.<br>Optional bounded Recent / Relevant Activity only when an existing read model supports it. |
| `P18.9.2` | `allowed_paths` | `2_products/pepper-agent/web/src/App.tsx`<br>`2_products/pepper-agent/web/src/agent-platform/extensions.ts`<br>`2_products/pepper-agent/web/src/agent-platform/extensions.test.ts`<br>`2_products/pepper-agent/web/src/agent-platform/shell/**`<br>`2_products/pepper-agent/web/src/agent-platform/runtime-overview/**`<br>`2_products/pepper-agent/tests/hermes_cli/test_agent_platform_ticket_architect_bridge.py` |
| `P18.9.2` | `allowed_actions` | Reuse the existing P18.9.1 shell, routing, navigation, plugin-protection, and static extension architecture for the canonical Overview route.<br>Compose existing Pepper/Hermes read models for current project, macroproject, current/generated ticket, workflow control, next action, approvals, execution status, review state, validation state, recovery state, and human Git handoff state.<br>Add focused behavioral validation for Overview route, CONTROL ordering, Current Work, Next Governed Action, Needs Attention, execution summaries, governed-state rendering, unavailable/error handling, downstream-boundary preservation, navigation regression, frontend typecheck, and production build. |
| `P18.9.2` | `constraints` | Do not create a second workflow state machine, dashboard state store, router, plugin loader, execution model, approval model, or backend authority.<br>Do not implement full Projects/Tickets tables or editor; P18.9.4 owns that workspace.<br>Do not implement an approval decision workspace; P18.9.5 owns that experience.<br>Do not implement a complete execution inspector, review workflow, recovery workflow, or Git-handoff workflow UI; P18.9.6 owns those surfaces.<br>Do not redesign Lead Agent; P18.9.3 owns that product experience.<br>Do not create a synthetic health score or require decorative charts.<br>Preserve CONTROL with Overview before Lead Agent and preserve WORK, AGENTS, AUTOMATION, RESOURCES, and SYSTEM primary groups.<br>Do not change backend API authority, provider behavior, worker dispatch, Kanban projection, Docker, Graphify, or Git authority. |
| `P18.9.2` | `non_goals` | No second workflow-control source.<br>No duplicate approval or execution authority.<br>No alternate Overview route or router.<br>No full downstream Work, Approval, Execution, Review, Recovery, Git handoff, or Lead Agent specialization UX.<br>No synthetic health score.<br>No Git, Docker, Graphify, provider, model, worker, or Kanban authority changes. |
| `P18.9.2` | `risk` | Overview scope drift into downstream P18.9.3-P18.9.6 specialist workspaces.<br>Navigation regression if Overview moves behind Lead Agent or exits CONTROL.<br>Authority regression if Overview invents state instead of composing existing read models.<br>Unavailable/error state regression if malformed read models render as trusted live state. |
| `P18.9.2` | `tasks` | Inventory the existing Overview route, descriptor, runtime-overview parser, runtime-overview view, shell group ordering, and P18.9.1 navigation tests before editing.<br>Implement the Overview composition using existing runtime overview and workflow-control read-model fields without adding backend authority.<br>Render Current Work, Next Governed Action, Needs Attention, Execution summary, and Governed State summary with explicit loading, no-current-work, idle-execution, active-execution, human-attention, blocked/recovery, unavailable, and error behavior.<br>Preserve `/agent-platform/overview`, keep Overview before Lead Agent in CONTROL, and preserve the accepted six Pepper primary groups.<br>Keep P18.9.3-P18.9.6 specialist UX out of the Overview while linking or summarizing bounded state when appropriate.<br>Add focused tests for the route, composition, states, read-model reuse, navigation preservation, and downstream-boundary contracts. |
| `P18.9.2` | `acceptance_criteria` | TicketSpec type is `implementation`, not P18.9.0 architecture/documentation-only planning.<br>The canonical Overview route remains `/agent-platform/overview` and no alternate route or router is created.<br>Overview remains before Lead Agent in CONTROL and the six Pepper primary groups remain CONTROL, WORK, AGENTS, AUTOMATION, RESOURCES, and SYSTEM.<br>Overview answers the five questions: what am I working on, what state is it in, does anything require human attention, is Pepper executing anything now, and what is the next governed action.<br>Overview renders Current Work, Next Governed Action, Needs Attention, Execution summary, and Governed State summary from existing read models.<br>Loading, no-current-work, idle-execution, active-execution, human-attention, blocked/recovery, unavailable, and error read-model states have deterministic behavior.<br>The implementation reuses existing current project, macroproject, current/generated ticket, workflow control, next action, approvals, execution status, review state, validation state, recovery state, and human Git handoff read models.<br>The implementation does not duplicate Projects/Tickets, Approval, Execution, Review, Recovery, Git handoff, or Lead Agent specialist UX reserved for P18.9.3-P18.9.6.<br>No backend API, provider, worker, Kanban, Docker, Graphify, or Git authority changes are introduced. |
| `P18.9.2` | `expected_artifacts` | Overview route and runtime-overview component/parser changes inside allowed paths.<br>Focused frontend tests proving canonical route, CONTROL ordering, Current Work rendering, Next Governed Action rendering, Needs Attention behavior, active and idle execution summaries, governed-state rendering, unavailable/error behavior, no downstream duplication, and no navigation regression.<br>Frontend typecheck and production build evidence.<br>Final report with files inspected, files modified, tests run, preserved read-model reuse, preserved downstream boundaries, and rollback posture. |
| `P18.9.2` | `validation_steps` | V1: Canonical Overview route validates => `/agent-platform/overview` resolves through the existing product extension descriptor without adding an alternate route or router.<br>V2: Overview remains before Lead Agent in CONTROL => Navigation tests show CONTROL renders Overview before Lead Agent and preserves WORK, AGENTS, AUTOMATION, RESOURCES, and SYSTEM.<br>V3: Current Work rendering validates => Tests show current project, macroproject, current/generated ticket, workflow status, review, validation, recovery, and human Git handoff state are rendered from existing read models.<br>V4: Next Governed Action rendering validates => Tests show workflow-control next action appears without inventing a second action authority.<br>V5: Needs Attention behavior validates => Tests cover pending approvals, pending ticket approval count, human attention required, blocked/recovery, unavailable, and no-attention states.<br>V6: Execution summary validates => Tests cover active execution and idle execution summaries from existing execution read models.<br>V7: Governed State summary validates => Tests cover workflow status, queue state, approval state, review state, validation state, recovery state, and human Git handoff state.<br>V8: Downstream boundaries validate => Tests or source review show no full Projects/Tickets workspace, approval decision workspace, execution inspector, review/recovery/Git-handoff workflow UI, or Lead Agent redesign is implemented.<br>V9: Frontend validation completes => Focused frontend tests, frontend typecheck, and production build pass or report bounded environment blockers without changing authority. |
| `P18.9.2` | `completion_verdict` | `p18_9_2_control_center_overview_implementation_ready` |
| `P18.9.2` | `recommended_commit_message` | `P18.9.2 Implement Control Center Overview` |
