...
P12.C3 preserves `2_products` as a bounded product class but replaces its
historical whole-class ignore with a fail-closed per-product rule. All children
remain ignored by default. Only the absent future `2_products/hermes-agent` path
is currently excepted for normal AGENT PLATFORM main-repository tracking. The
exception creates no source, activates no product, permits no nested Git or
independent remote, and grants no sibling exception.
...
| `2_products/` | Folder | Product workspace class; ignored/local-only by default | Contains six ignored candidate siblings. Future `hermes-agent` is absent and is the sole P12.C3 tracking exception. |
...
| `2_products/` | Read by product/migration ticket. | Write by product ticket only. | Execution prohibited unless explicitly approved. | Ignored/local-only by default; future `hermes-agent` alone is normally trackable after its gates. | Required before product commits or publication. |
...
- Product folders are ignored by default. P12.C3 changes that posture only for the absent future `2_products/hermes-agent`; every sibling remains local-only until its own decision.
...
| Product folders are ignored by default but active as candidates | Six current siblings remain ignored; future `hermes-agent` is the sole P12.C3 tracking exception. | Product work may need versioned source later. | Decide every product Git posture explicitly before implementation. |
...
| What must remain local-only? | `.git/`, Office temp/metadata files, `DT.xlsx`, every `2_products` child except future `hermes-agent`, `4_external/sources/`, `7_datasets/`, `8_models/`, `9_artifacts/`, runtime/generated folders named in `.gitignore`, and `previusknowledge/` until governance decides otherwise. |
