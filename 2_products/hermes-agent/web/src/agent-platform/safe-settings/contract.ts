...
export interface SafePreferenceWriteRequest {
  readonly preferenceId: string;
  readonly valueId: string;
}

export interface SafePreferenceWriteResult {
  readonly ok: boolean;
  readonly preferenceId: string;
...
export interface SafePreferenceWriteState {
...
export function validateSafePreferenceWriteRequest(
  snapshot: SafeSettingsSnapshot,
  value: unknown,
): SafePreferenceWriteRequest {
  const source = record(value, "safe preference write request");
  exactKeys(source, ["preferenceId", "valueId"], "safe preference write request");
  const request = Object.freeze({
    preferenceId: identifier(source.preferenceId, "preferenceId"),
    valueId: identifier(source.valueId, "valueId"),
  });
  const preference = snapshot.preferences.find((item) => item.id === request.preferenceId);
  if (
    !preference ||
    preference.classification !== "safe_read_write" ||
    !preference.writeAvailable
  ) {
    throw new Error("Preference is not available for safe writes");
  }
  if (!preference.allowedValues.some((item) => item.id === request.valueId)) {
    throw new Error("Value is not in the trusted preference allowlist");
  }
  return request;
}
...
export const INITIAL_SAFE_PREFERENCE_WRITE_STATE: SafePreferenceWriteState = Object.freeze({
  phase: "idle",
  preferenceId: null,
  draftValueId: null,
