...
describe("qualified GET-only execution client", () => {
  ...
  it("contains no mutation endpoint or independent execution API", () => {
    const source = readFileSync(new URL("./execution-client.ts", import.meta.url), "utf8");
    expect(source).not.toMatch(/\b(?:POST|PUT|PATCH|DELETE)\b/);
    expect(source).not.toMatch(/\/v1\/runs|\/inspect|\/terminate|\/stop|\/retry|\/rollback/);
  });
});
...
describe("read-only Execution Inspector pages", () => {
  ...
  it("renders only safe detail facts and explicitly linked event evidence without controls", () => {
    ...
    expect(markup).toContain("No stop, retry, rollback or execution action is available in P13.6");
