...
def test_registry_contains_exact_two_profiles_in_deterministic_order() -> None:
    ...
    assert list_runtime_profile_ids() == (
        ...
        ra.HERMES_DASHBOARD_EXPERIMENTAL_PROFILE_ID,
    )
    assert (
    ...
    assert get_runtime_profile(ra.HERMES_DASHBOARD_EXPERIMENTAL_PROFILE_ID) is listed[1]
...
def test_profile_definitions_are_immutable_and_have_safe_repr() -> None:
    ...
    assert "dashboard" in repr(
        get_runtime_profile(ra.HERMES_DASHBOARD_EXPERIMENTAL_PROFILE_ID)
    )
...
def test_dashboard_experimental_profile_matches_contract() -> None:
    profile = get_runtime_profile(ra.HERMES_DASHBOARD_EXPERIMENTAL_PROFILE_ID)
    ref = profile.profile_ref

    assert ref.profile_class is ra.RuntimeProfileClass.HERMES_DASHBOARD_EXPERIMENTAL
    assert (
        profile.executable_selector is RuntimeExecutableSelector.CURRENT_PRODUCT_PYTHON
    )
    assert (
        profile.argument_selector
        is RuntimeArgumentSelector.HERMES_DASHBOARD_EXPERIMENTAL
    )
