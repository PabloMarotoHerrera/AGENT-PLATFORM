...
_BOUNDARY_SOURCE = "agent-platform:chatgpt_oauth_device"
...
class StoreProtectionBackend:
    ...
    def validate_file(self, path: Path) -> StoreProtectionReport:
        ...
        return StoreProtectionReport(
            path_role="auth_file", platform="posix", protected=True
        )
...
def _validate_windows_dacl(path: Path, *, path_role: str) -> StoreProtectionReport:
    ...
    try:
        ...
        validate_windows_dacl_principals(
            principal_sids, current_user_sid=current_user_sid
        )
    except ProviderCredentialStoreError:
        raise
    except Exception as exc:
    ...
    try:
        ...
        for index in range(info.AceCount):
            ...
            if header.AceType != ACCESS_ALLOWED_ACE_TYPE:
                continue
            sid_pointer = int(ace.value) + 8
            allowed_sids.add(_sid_to_string(sid_pointer))
        return allowed_sids
    finally:
...
def _build_auth_store(credential: OpenAICodexOAuthCredential) -> dict[str, Any]:
    ...
    expires_at = _format_utc(credential.expires_at_utc)
    entry = {
...
def validate_openai_codex_auth_store_payload(
    ...
    """Validate the exact dedicated Hermes auth shape and return the credential."""
    ...
    if set(providers) != {OPENAI_CODEX_HERMES_PROVIDER_ID}:
        raise InvalidProviderCredentialStoreError(
            validation_category="unexpected_provider"
        )
    provider_state = _require_dict(
        providers.get(OPENAI_CODEX_HERMES_PROVIDER_ID),
        "provider_state_not_object",
    )
    if provider_state.get("schema_version") != PROVIDER_CREDENTIAL_SCHEMA_VERSION:
        raise InvalidProviderCredentialStoreError(
            validation_category="schema_version_mismatch"
        )
    if provider_state.get("credential_store_id") != OPENAI_CODEX_CREDENTIAL_STORE_ID:
        raise InvalidProviderCredentialStoreError(
            validation_category="store_id_mismatch"
        )
    if provider_state.get("auth_mode") != "chatgpt":
        raise InvalidProviderCredentialStoreError(
            validation_category="auth_mode_mismatch"
        )
    if provider_state.get("base_url") != OPENAI_CODEX_PROVIDER_ENDPOINT:
        raise InvalidProviderCredentialStoreError(
            validation_category="base_url_mismatch"
        )
    if provider_state.get("label") != OPENAI_CODEX_INTERNAL_LABEL:
        raise InvalidProviderCredentialStoreError(validation_category="label_mismatch")
    if provider_state.get("source") != _BOUNDARY_SOURCE:
        raise InvalidProviderCredentialStoreError(validation_category="source_mismatch")
    last_refresh = _parse_utc(provider_state.get("last_refresh"))
    expires_at = _parse_utc(provider_state.get("expires_at"))
    tokens = _require_dict(provider_state.get("tokens"), "tokens_not_object")
    access_token = _require_token(tokens.get("access_token"), "access_token_missing")
    refresh_token = _require_token(tokens.get("refresh_token"), "refresh_token_missing")
    ...
    if entry.get("id") != OPENAI_CODEX_CREDENTIAL_STORE_ID:
        raise InvalidProviderCredentialStoreError(
    ...
    if entry.get("label") != OPENAI_CODEX_INTERNAL_LABEL:
        raise InvalidProviderCredentialStoreError(
            validation_category="pool_label_mismatch"
        )
    if (
        entry.get("access_token") != access_token
        or entry.get("refresh_token") != refresh_token
    ):
        raise InvalidProviderCredentialStoreError(
            validation_category="pool_token_mismatch"
        )
    if entry.get("last_refresh") != provider_state.get("last_refresh"):
        raise InvalidProviderCredentialStoreError(
            validation_category="pool_last_refresh_mismatch"
        )
    if entry.get("expires_at") != provider_state.get("expires_at"):
        raise InvalidProviderCredentialStoreError(
            validation_category="pool_expiry_mismatch"
        )
...
def _status_from_valid_store(
    *,
    ...
    return ProviderCredentialStatus(
        ...
        provider_state_present=credential is not None,
...
def read_openai_codex_credential_status(
    ...
    *,
    ...
    credential = validate_openai_codex_auth_store_payload(
...
def load_openai_codex_oauth_credential(
    ...
    *,
    ...
    backend.validate_directory(auth_file.parent)
    backend.validate_file(auth_file)
    return validate_openai_codex_auth_store_payload(_load_json_auth_store(auth_file))
...
def promote_openai_codex_oauth_credential(
    ...
    *,
    ...
    auth_store = _build_auth_store(credential)
    ...
    try:
        ...
        validate_openai_codex_auth_store_payload(_load_json_auth_store(staging_file))
...
def clear_local_openai_codex_credential(
    ...
    *,
    ...
    backend.validate_directory(auth_file.parent)
    backend.validate_file(auth_file)
    validate_openai_codex_auth_store_payload(_load_json_auth_store(auth_file))
    try:
        auth_file.unlink()
