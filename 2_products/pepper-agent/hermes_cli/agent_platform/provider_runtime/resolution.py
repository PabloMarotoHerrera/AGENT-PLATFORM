"""Internal safe resolver for P15.M8 provider-runtime profile metadata."""
...
class ProviderRuntimeResolutionError(ValueError):
    """Raised when supplied metadata violates the P15.M8 profile."""
...
def resolve_provider_runtime_profile(
    ...
    *,
    ...
    if status.client_token_status is None:
        raise ProviderRuntimeResolutionError(
            "client_token_metadata_required",
            "provider-runtime readiness requires client-token metadata",
        )
    if not status.client_token_status.usable_for_bounded_lease:
        raise ProviderRuntimeResolutionError(
            "client_token_not_lease_usable",
            "client-token metadata is not usable for a bounded lease",
        )
