...
TICKET_SPEC_MATERIAL_REVISION_CONTRACT_SCHEMA_VERSION = 1
TICKET_SPEC_MATERIAL_REVISION_CONTRACT_DIGEST_ALGORITHM = (
    "agent-platform-ticket-spec-material-revision-contract-sha256-v1"
)
...
def _reject_structured_revision_control_text(value: str, *, field_name: str) -> str:
...
def _normalized_structured_revision_text(value: str) -> str:
...
class TicketSpecMaterialRevisionContract(BaseModel):
    ...
    def _validate_text_entries(
        ...
        checked = tuple(
            _reject_structured_revision_control_text(
                item,
                field_name="structured revision entry",
            )
            for item in value
        )
        normalized = tuple(_normalized_structured_revision_text(item) for item in checked)
        if len(normalized) != len(frozenset(normalized)):
            raise ValueError("structured revision entries must be unique after normalization")
        return checked
...
def current_ticket_material_revision_history_path_for_ticket(ticket_id: str) -> Path:
    """Return the append-only current-ticket material revision history path."""

    safe_ticket_id = _safe_ticket_id(ticket_id)
    return get_hermes_home() / _REVISION_STORE_DIR / f"{safe_ticket_id}.current-ticket.jsonl"
...
def load_immutable_rejected_current_ticket_authority(
    *,
    ticket_id: str,
    require_rejected_decision: bool = False,
) -> dict[str, dict[str, Any]] | None:
    """Load rejected current-ticket correction base authority without roadmap reconstruction."""

    return _load_immutable_decided_ticket_authority(
        ticket_id=ticket_id,
        require_decision=require_rejected_decision,
        expected_decision=HumanApprovalDecision.REJECT,
        expected_status="rejected",
        missing_decision_message=f"{_safe_ticket_id(ticket_id)} rejected ticket decision is absent",
        wrong_decision_message=f"{_safe_ticket_id(ticket_id)} current ticket is not rejected",
        wrong_status_message=f"{_safe_ticket_id(ticket_id)} current ticket rejection status mismatch",
    )
...
def _load_immutable_approved_ticket_authority(
    *,
    ...
    return _load_immutable_decided_ticket_authority(
        ticket_id=ticket_id,
        require_decision=require_approved_decision,
        expected_decision=HumanApprovalDecision.APPROVE,
        expected_status="approved",
        missing_decision_message=missing_decision_message,
        wrong_decision_message=not_approved_message,
        wrong_status_message=not_approved_status_message,
    )


def _load_immutable_decided_ticket_authority(
    *,
    ticket_id: str,
    require_decision: bool,
    expected_decision: HumanApprovalDecision,
    expected_status: str,
    missing_decision_message: str,
    wrong_decision_message: str,
    ...
    if decision is None:
        ...
        return None
    if decision.get("decision") != expected_decision.value:
        if require_decision:
            raise TicketArchitectBridgeConflict(wrong_decision_message)
        return None
    if decision.get("status") != expected_status:
        if require_decision:
            raise TicketArchitectBridgeConflict(wrong_status_message)
        return None
    try:
        validated_decision = _validate_approval_decision_record_with_generation(
            decision,
            ticket_id=safe_ticket_id,
            generation=generation,
        )
        ...
        if require_decision:
            raise
        raise TicketArchitectBridgeConflict(
            f"{safe_ticket_id} {expected_status} ticket decision is invalid"
        )
    return {
        "generation_record": generation,
        "approval_decision_record": validated_decision,
    }
...
def revise_current_ticket_for_material_contract_failure(
    *,
    ...
    """Revise an active current ticket after governed material or rejection authority."""

    if revision_contract is None:
        raise TicketArchitectBridgeInputError("current-ticket material revision requires revision_contract")
    if not isinstance(workflow, dict):
        raise TicketArchitectBridgeInputError("workflow state is unavailable")
    next_action = workflow.get("next_action")
    if not isinstance(next_action, dict):
        next_action = {}
    safe_ticket_id = _safe_ticket_id(
        requested_ticket_id
        or workflow.get("current_ticket_id")
        or next_action.get("target_ticket_id")
    )
    current_generation: dict[str, Any] | None = None
    ...
    with _STORE_LOCK:
        ...
        if approved_authority is None:
            rejected_authority = load_immutable_rejected_current_ticket_authority(
                ticket_id=safe_ticket_id,
                require_rejected_decision=False,
            )
        if approved_authority is None and rejected_authority is None:
        ...
        current_authority = approved_authority or rejected_authority
        assert current_authority is not None
        current_generation = current_authority["generation_record"]
        previous_decision = current_authority["approval_decision_record"]
        if rejected_authority is not None:
            revision_reason = "rejected_current_ticket_revision_correction"
        ...
        _validate_current_ticket_material_revision_workflow(
            workflow,
            target=target,
        ...
        try:
            ...
            pass
        _append_current_ticket_material_revision_history_entry(
            target.ticket_id,
            history_entry,
        )
    result = _operational_result(revised_generation, idempotent_replay=False)
    result.update({
        ...
        "human_authorization_text": human_authorization_text,
        "authorizer_id": authorizer_id,
        "previous_generation_bridge_SHA256": current_generation["bridge_SHA256"],
        "previous_approval_publication_SHA256": previous_decision[
            "approval_publication_SHA256"
        ],
        "previous_approval_decision": previous_decision["decision"],
        "historical_current_generation_preserved": True,
        "historical_previous_decision_preserved": True,
        "revision_history_path": str(
            current_ticket_material_revision_history_path_for_ticket(target.ticket_id)
        ),
        "revision_SHA256": history_entry["revision_SHA256"],
        "revision_contract_SHA256": revision_authority.get("revision_contract_SHA256"),
        "revision_authority_accepted": True,
        "revision_authority_recorded": True,
        "revision_contract_supplied": True,
        "revision_contract_accepted": validated_revision_contract is not None,
        "revision_contract_recorded": True,
        "revision_contract_applied": True,
        "successor_publication_created": True,
        "successor_publication_persisted": True,
        "successor_generated": True,
        "pending_ticket_approval_count": 1,
    ...
    if revision_reason == "material_contract_failure":
        assert review_prepare_failure_record is not None
        result.update({
            "review_prepare_failure_SHA256": review_prepare_failure_record[
...
def _validate_current_ticket_material_revision_workflow(
    workflow: dict[str, Any],
    *,
    target: GovernedTicketGenerationTarget,
    review_prepare_failure_record: dict[str, Any] | None,
    ...
    if workflow.get("current_ticket_id") != target.ticket_id:
        raise TicketArchitectBridgeInputError(
            f"current-ticket revision requires active ticket {target.ticket_id}"
        )
    next_action = workflow.get("next_action")
    if not isinstance(next_action, dict):
        raise TicketArchitectBridgeInputError("next action is unavailable")
    ...
    if int(workflow.get("active_execution_count") or 0) != 0:
        raise TicketArchitectBridgeInputError("an execution is already active")
    if revision_reason == "rejected_current_ticket_revision_correction":
        if workflow.get("workflow_status") != "awaiting_correction":
            raise TicketArchitectBridgeInputError(
                f"{target.ticket_id} rejected current-ticket revision requires awaiting_correction"
            )
        if workflow.get("workflow_state") != f"{target.ticket_id}-AWAITING-CORRECTION":
            raise TicketArchitectBridgeInputError(
                f"workflow state is not {target.ticket_id}-AWAITING-CORRECTION"
    ...
    if workflow.get("workflow_status") != "awaiting_material_revision":
        raise TicketArchitectBridgeInputError(
            f"{target.ticket_id} material revision requires awaiting_material_revision"
        )
    if workflow.get("workflow_state") != f"{target.ticket_id}-AWAITING-MATERIAL-REVISION":
        raise TicketArchitectBridgeInputError(
            f"workflow state is not {target.ticket_id}-AWAITING-MATERIAL-REVISION"
        )
    if workflow.get("material_revision_required") is not True:
    ...
    if review_prepare_failure_record.get("review_prepare_resolution") != "MATERIAL_REVISION_REQUIRED":
        raise TicketArchitectBridgeInputError("review-prepare failure does not require material revision")
    if review_prepare_failure_record.get("material_revision_required") is not True:
        raise TicketArchitectBridgeInputError("review-prepare failure does not authorize material revision")
    if review_prepare_failure_record.get("ticket_id") != target.ticket_id:
        raise TicketArchitectBridgeInputError("review-prepare failure ticket mismatch")
    if review_prepare_failure_record.get("ticket_spec_SHA256") != current_generation.get("ticket_spec_SHA256"):
        raise TicketArchitectBridgeInputError("review-prepare failure TicketSpec mismatch")
    if review_prepare_failure_record.get("work_packet_id") != current_generation.get("work_packet_id"):
        raise TicketArchitectBridgeInputError("review-prepare failure WorkPacket id mismatch")
    if review_prepare_failure_record.get("work_packet_SHA256") != current_generation.get("work_packet_SHA256"):
        raise TicketArchitectBridgeInputError("review-prepare failure WorkPacket digest mismatch")
    projected_failure = workflow.get("review_prepare_failure_authority")
    if not isinstance(projected_failure, dict):
        raise TicketArchitectBridgeInputError("workflow failure authority projection is unavailable")
    if projected_failure.get("review_prepare_failure_SHA256") != review_prepare_failure_record.get(
        "review_prepare_failure_SHA256"
    ):
        raise TicketArchitectBridgeInputError("workflow failure authority projection mismatch")
...
def validate_ticket_spec_material_revision_contract(
    value: object,
    *,
    target: GovernedTicketGenerationTarget,
) -> TicketSpecMaterialRevisionContract:
    try:
        contract = TicketSpecMaterialRevisionContract.model_validate(value)
    except ValueError as exc:
        raise TicketArchitectBridgeInputError(
            "structured TicketSpec revision contract is invalid"
        ) from exc
    if contract.ticket_id != target.ticket_id:
        raise TicketArchitectBridgeInputError(
    ...
    if len(encoded) > _MATERIAL_REVISION_CONTRACT_MAX_SERIALIZED_CHARS:
        raise TicketArchitectBridgeInputError(
            "structured TicketSpec revision contract is too large"
        )
    return contract


def ticket_spec_material_revision_contract_digest(
    contract: TicketSpecMaterialRevisionContract | dict[str, Any],
) -> str:
    validated = (
        contract
        if isinstance(contract, TicketSpecMaterialRevisionContract)
        else TicketSpecMaterialRevisionContract.model_validate(contract)
    )
    encoded = json.dumps(
        {
            "algorithm": TICKET_SPEC_MATERIAL_REVISION_CONTRACT_DIGEST_ALGORITHM,
            "contract": _normalize(validated),
        },
        ensure_ascii=False,
        separators=(",", ":"),
        sort_keys=True,
...
def _revision_contract_from_authority(
    ...
    *,
    target: GovernedTicketGenerationTarget,
) -> TicketSpecMaterialRevisionContract | None:
    raw_contract = authority.get("revision_contract")
    raw_digest = authority.get("revision_contract_SHA256")
    if raw_contract is None and raw_digest is None:
        return None
    if raw_contract is None or raw_digest is None:
        raise TicketArchitectBridgeConflict("revision contract authority is incomplete")
    try:
        contract = validate_ticket_spec_material_revision_contract(
            raw_contract,
            target=target,
        )
    except TicketArchitectBridgeInputError as exc:
        raise TicketArchitectBridgeConflict(
            "revision contract authority is invalid"
        ) from exc
    if raw_digest != ticket_spec_material_revision_contract_digest(contract):
        raise TicketArchitectBridgeConflict("revision contract digest mismatch")
    return contract
...
def _build_material_revision_authority(
    *,
    ...
    if validated_contract is not None:
        authority["revision_contract"] = validated_contract.model_dump(mode="json")
        authority["revision_contract_SHA256"] = ticket_spec_material_revision_contract_digest(
            validated_contract
        )
...
def _build_current_ticket_material_revision_authority(
    *,
    current_generation: dict[str, Any],
    ...
    if revision_reason not in {
        ...
        raise TicketArchitectBridgeInputError("current-ticket revision reason is unsupported")
    if revision_reason == "material_contract_failure" and not isinstance(
        review_prepare_failure_record,
        dict,
    ):
        raise TicketArchitectBridgeInputError("review-prepare failure authority is unavailable")
    previous_publication = _publication_from_generation_record(current_generation)
    base_ticket_spec = _revision_base_ticket_spec_from_generation(
        current_generation,
        target=target,
    )
    previous_revision = int(previous_publication.revision)
    validated_contract = validate_ticket_spec_material_revision_contract(
        revision_contract,
        target=target,
    )
    authority = {
        "schema_version": TICKET_ARCHITECT_BRIDGE_SCHEMA_VERSION,
        "policy_id": TICKET_ARCHITECT_MATERIAL_REVISION_AUTHORITY_POLICY_ID,
        "authority_type": "current_ticket_material_contract_revision",
        "revision_reason": revision_reason,
        "project_id": target.project_id,
        "macroproject_id": target.macroproject_id,
        "ticket_id": target.ticket_id,
        ...
        "roadmap_authority_path": target.roadmap_authority_path,
        "roadmap_authority_section": target.roadmap_authority_section,
        "previous_bridge_SHA256": current_generation["bridge_SHA256"],
        "previous_ticket_spec_SHA256": current_generation["ticket_spec_SHA256"],
        "revision_base_ticket_spec": base_ticket_spec.model_dump(mode="json"),
        "revision_base_ticket_spec_SHA256": current_generation["ticket_spec_SHA256"],
        "previous_dependency_plan_SHA256": current_generation["dependency_plan_SHA256"],
        "previous_lint_report_SHA256": current_generation["lint_report_SHA256"],
        "previous_work_packet_id": current_generation["work_packet_id"],
        "previous_work_packet_SHA256": current_generation["work_packet_SHA256"],
        "previous_publication_id": previous_publication.publication_id,
        "previous_publication_revision": previous_revision,
        "new_publication_revision": previous_revision + 1,
        "human_authorization_text": human_authorization_text,
        "authorizer_id": authorizer_id,
        "revision_contract": validated_contract.model_dump(mode="json"),
        "revision_contract_SHA256": ticket_spec_material_revision_contract_digest(
            validated_contract
        ),
    }
    if revision_reason == "material_contract_failure":
        assert review_prepare_failure_record is not None
        authority.update({
            "approved_approval_publication_SHA256": previous_decision[
                "approval_publication_SHA256"
            ],
            ...
            "review_prepare_validation_result_SHA256": review_prepare_failure_record.get(
                "review_prepare_validation_result_SHA256"
            ),
            "review_prepare_resolution": review_prepare_failure_record[
        ...
        authority.update({
            ...
            "rejected_approval_decision": previous_decision["decision"],
        })
    authority["revision_authority_SHA256"] = _revision_authority_digest(authority)
    return authority
...
def _validate_material_revision_authority(
    ...
    *,
    ...
    if authority_type not in {
        "rejected_successor_material_revision",
        "current_ticket_material_contract_revision",
    }:
        raise TicketArchitectBridgeConflict("revision authority type mismatch")
    expected = {
        "schema_version": TICKET_ARCHITECT_BRIDGE_SCHEMA_VERSION,
        "policy_id": TICKET_ARCHITECT_MATERIAL_REVISION_AUTHORITY_POLICY_ID,
        "authority_type": authority_type,
    ...
    if authority_type == "rejected_successor_material_revision":
        ...
        if revision_reason == "material_contract_failure":
            if authority.get("approved_approval_decision") != HumanApprovalDecision.APPROVE.value:
                raise TicketArchitectBridgeConflict("revision authority approved decision mismatch")
    ...
    approval_digest_field = (
        ...
        if authority_type == "rejected_successor_material_revision"
        or authority.get("revision_reason") == "rejected_current_ticket_revision_correction"
        else "approved_approval_publication_SHA256"
    )
    _safe_digest(authority.get(approval_digest_field))
    if authority.get("revision_base_ticket_spec") is not None:
        _safe_digest(authority.get("revision_base_ticket_spec_SHA256"))
    return authority


def _next_revision_validation_id(
    validation_steps: tuple[TicketValidationStepSpec, ...],
) -> str:
    max_index = 0
    for step in validation_steps:
        match = re.fullmatch(r"V([1-9][0-9]*)", step.validation_id)
        if match:
            max_index = max(max_index, int(match.group(1)))
    return f"V{max_index + 1}"
...
def _build_revision_ticket_spec(
    ...
    *,
    ...
    current_ticket_revision = (
        authority.get("authority_type") == "current_ticket_material_contract_revision"
    )
    current_ticket_revision_reason = str(authority.get("revision_reason") or "")
    current_rejected_revision = (
        current_ticket_revision
        and current_ticket_revision_reason == "rejected_current_ticket_revision_correction"
    )
    revision_contract = _revision_contract_from_authority(authority, target=target)
    base_commit_message = (
    ...
    revision_refs = (
        AuthorityReferenceSpec(
            ...
            rationale=(
                "Explicit human correction authority for the rejected current-ticket revision."
                if current_rejected_revision
                else "Explicit human material-revision authority for the current ticket."
                if current_ticket_revision
                else "Explicit human correction authority for the material generated-successor revision."
            ),
        ),
        AuthorityReferenceSpec(
            kind=AuthorityReferenceKind.GOVERNANCE_RECORD,
            ...
            rationale=(
                ...
                if current_ticket_revision
                else "Rejected predecessor generated-ticket authority remains immutable historical evidence."
            ),
        ),
    )
    if revision_contract is not None:
        ...
        revision_context = (
            (
                ...
                f"{authority['revision_contract_SHA256']} overlays the prior TicketSpec; "
                "omitted fields inherit unchanged."
            ),
        )
    else:
        product_objective = base.objective
        product_context = base.context
        product_scope = base.scope
        product_constraints = base.constraints
    ...
    return TicketSpec(
        ...
        validation_steps=product_validation_steps
        + (
            TicketValidationStepSpec(
...
def _build_generation_record(
    ...
    *,
    ...
    if revision_authority is not None:
        ...
        if authority.get("revision_contract_SHA256") is not None:
            record["revision_contract_SHA256"] = authority["revision_contract_SHA256"]
...
def _assemble_context_pack(
    ...
    *,
    ...
    if revision_authority is not None:
        ...
        authority_type = str(authority.get("authority_type") or "")
        revision_reason = str(authority.get("revision_reason") or "")
        if authority_type == "current_ticket_material_contract_revision":
            revision_source = (
                "after a rejected current-ticket revision"
                if revision_reason == "rejected_current_ticket_revision_correction"
                else "after a durable review-preparation material contract failure"
            )
        else:
            revision_source = "after a rejected generated-successor approval"
        contract_fragment = (
            f" Structured contract SHA256: {authority['revision_contract_SHA256']}."
            if authority.get("revision_contract_SHA256")
            else ""
        )
        sources.append(
            ContextSourceSpec(
                ...
                content=(
                    ...
                    f"supersedes {authority['previous_publication_id']} {revision_source}."
                    f"{contract_fragment} Human correction: {authority['human_authorization_text']}"
                ),
...
def _build_rejected_successor_revision_history_entry(
    *,
    rejected_generation: dict[str, Any],
    rejected_decision: dict[str, Any],
    revised_generation: dict[str, Any],
    ...
    if isinstance(revision_authority, dict) and revision_authority.get(
        "revision_contract_SHA256"
    ):
        entry["revision_contract_SHA256"] = revision_authority[
            "revision_contract_SHA256"
        ]
        entry["revision_contract"] = revision_authority["revision_contract"]
    entry["revision_SHA256"] = _revision_history_record_digest(entry)
    return entry


def _build_current_ticket_material_revision_history_entry(
    *,
    current_generation: dict[str, Any],
    previous_decision: dict[str, Any],
    review_prepare_failure_record: dict[str, Any] | None,
    ...
    entry = {
        ...
        "revised_work_packet_id": revised_generation["work_packet_id"],
        "revised_work_packet_SHA256": revised_generation["work_packet_SHA256"],
        "historical_current_generation_record": current_generation,
        "new_generation_record": revised_generation,
        "ticket_generated": True,
        "human_ticket_approval_required": True,
        "human_ticket_approval_present": False,
...
def _append_rejected_successor_revision_history_entry(
    ticket_id: str,
    entry: dict[str, Any],
) -> None:
    history_path = rejected_successor_revision_history_path_for_ticket(ticket_id)
    history_path.parent.mkdir(parents=True, exist_ok=True)
    with history_path.open("a", encoding="utf-8") as handle:
        handle.write(json.dumps(entry, ensure_ascii=False, sort_keys=True) + "\n")


def _append_current_ticket_material_revision_history_entry(
    ticket_id: str,
    entry: dict[str, Any],
) -> None:
    history_path = current_ticket_material_revision_history_path_for_ticket(ticket_id)
    history_path.parent.mkdir(parents=True, exist_ok=True)
    with history_path.open("a", encoding="utf-8") as handle:
        handle.write(json.dumps(entry, ensure_ascii=False, sort_keys=True) + "\n")
...
def _authority_projection(record: dict[str, Any]) -> dict[str, Any]:
    ...
    if record.get("revision_contract_SHA256") is not None:
        projection["revision_contract_SHA256"] = record["revision_contract_SHA256"]
    return projection
...
__all__ = (
    ...
    "TICKET_APPROVAL_PUBLICATION_POLICY_ID",
    "TICKET_SPEC_MATERIAL_REVISION_CONTRACT_SCHEMA_VERSION",
    ...
    "current_ticket_material_revision_history_path_for_ticket",
    ...
    "load_immutable_rejected_current_ticket_authority",
    "validate_generation_record",
    "validate_p18_9_0_generation_record",
    "validate_approval_decision_record",
    "validate_p18_9_0_approval_decision_record",
    "validate_ticket_spec_material_revision_contract",
    "ticket_spec_material_revision_contract_digest",
