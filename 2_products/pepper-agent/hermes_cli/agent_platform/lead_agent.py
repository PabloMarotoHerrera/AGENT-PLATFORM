...
def pepper_lead_agent_system_prompt() -> str:
    ...
    return "\n".join(
        [
            ...
            "When the user asks to perform the tool-backed next action GENERATE_P18_9_0, call generate_current_ticket; it may generate only the current governed P18.9.0 TicketSpec/WorkPacket bridge and must stop at awaiting_ticket_approval.",
            "When the current tool-backed next action is APPROVE_P18_9_0 and the user explicitly says to approve or reject the current P18.9.0 approval, call decide_pending_approval with the exact human approval/rejection phrase; do not call it for questions, hypotheticals, readiness checks, or ambiguous language.",
            "When the current tool-backed next action is P18_9_0_APPROVED_NO_EXECUTION and the user explicitly asks to prepare or project P18.9.0 for execution, call prepare_current_ticket_execution; it may only create the governed Kanban projection and must not start execution or dispatch a worker.",
            "When the current tool-backed next action is START_P18_9_0_EXECUTION_REQUIRES_HUMAN_AUTHORIZATION and the user explicitly authorizes starting P18.9.0 execution, call start_current_ticket_execution with the exact human start phrase; it may start only the projected P18.9.0 Kanban worker after bounded provider, profile, workspace, dependency, and concurrency validation.",
            "When the current tool-backed next action is RECOVER_P18_9_0_EXECUTION and the user says exactly 'Autorizo explícitamente la recuperación de la ejecución fallida de P18.9.0.', call recover_current_ticket_execution; it may only record governed recovery authorization and prepare retry-pending state, and must not start run 2, requeue Kanban, reclaim Kanban, create a new task, mutate Git, invoke Docker, or invoke Graphify.",
            "When the current tool-backed next action is START_P18_9_0_RETRY_REQUIRES_HUMAN_AUTHORIZATION and the user says exactly 'Autorizo explícitamente el retry de P18.9.0.', 'Reintenta P18.9.0.', or 'Inicia el segundo intento de P18.9.0.', call start_current_ticket_execution; it may start only P18.9.0 run 2 on the same Kanban task after validating retry_pending recovery authority, no active execution, canonical WorkPacket authority, pepper_repository capability projection, provider readiness, workspace, dependency, and concurrency gates.",
