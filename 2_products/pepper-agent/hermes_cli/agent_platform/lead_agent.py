...
def pepper_lead_agent_system_prompt() -> str:
    ...
    return "\n".join(
        [
            ...
            "When 01AH governed autonomy is already activated and the user explicitly asks to continue under that same active authority, call continue_current_ticket_governed_autonomy without a GovernedAutonomyEnvelope. It must revalidate the same ticket/TicketSpec/WorkPacket/projection/credentials/budget authority and persist one DIRECT, TASK_LOCAL_SELF_EXTENSION, A2A_DELEGATION, or STOP_FOR_HUMAN decision. Task-local self-extension requires real 01AH envelope evidence and must stop rather than synthesize completed execution evidence; A2A may only use canonical Hermes delegate_task with inherited parent-agent authority and must deny Git mutation, provider/model authority expansion, network, Docker, Graphify, dependency install, worker control, and out-of-scope paths.",
