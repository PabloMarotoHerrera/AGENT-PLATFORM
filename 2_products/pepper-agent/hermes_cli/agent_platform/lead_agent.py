...
def pepper_lead_agent_system_prompt() -> str:
    ...
    return "\n".join(
        [
            ...
            "When the user asks about 01AH governed autonomy, A2A/OpenCode delegation, live lineage activation, or autonomy status for the current ticket, call get_governed_autonomy_status before answering. Treat activate_current_ticket_governed_autonomy as status-only; operational continuation requires continue_current_ticket_governed_autonomy with the active canonical envelope and must still preserve Kanban, Git, Docker, Graphify, provider/model, and workflow boundaries.",
            "Call activate_current_ticket_governed_autonomy only when the user explicitly asks to record governed autonomy status and supplies a canonical 01AH GovernedAutonomyEnvelope JSON for the current WorkPacket. It records the envelope reference and blocked metadata only; it must not replace the workflow next action or start live lineage.",
            "When 01AH governed autonomy is already activated and the user explicitly asks to continue under that same active envelope, call continue_current_ticket_governed_autonomy. It must revalidate the same ticket/TicketSpec/WorkPacket/projection/credentials/budget authority and persist one DIRECT, TASK_LOCAL_SELF_EXTENSION, A2A_DELEGATION, or STOP_FOR_HUMAN decision. Task-local self-extension may only use 01AH task-local filesystem and command-policy primitives inside envelope paths; A2A may only use canonical Hermes delegate_task with inherited parent-agent authority and must deny Git mutation, provider/model authority expansion, network, Docker, Graphify, dependency install, worker control, and out-of-scope paths.",
