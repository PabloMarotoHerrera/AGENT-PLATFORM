...
EXPECTED_ENUMS = {
    ...
    ra.RuntimeProfileClass: [
        ...
        "pepper_dashboard_provider_null",
    ],
    ra.RuntimeEventType: [
        "request_received",
        "request_rejected",
...
def test_vocabularies_are_exact_and_schema_version_is_stable() -> None:
    ...
    assert (
        ra.PEPPER_DASHBOARD_PROVIDER_NULL_PROFILE_ID == "pepper.dashboard.provider_null"
    )
...
def test_profile_ref_distinguishes_syntax_from_registration_and_records_files_root_invariant() -> (
    ...
    with pytest.raises(ValidationError):
        ra.RuntimeProfileRef(
            profile_id=ra.PEPPER_DASHBOARD_PROVIDER_NULL_PROFILE_ID,
            profile_class=ra.RuntimeProfileClass.PEPPER_DASHBOARD_PROVIDER_NULL,
