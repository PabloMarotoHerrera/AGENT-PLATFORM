...
def test_delivery_lease_has_pathless_public_ref_and_internal_projection(
    ...
    assert "synthetic-refresh-token" not in marker_text
...
def test_expired_store_and_near_expiry_lease_are_rejected(tmp_path: Path) -> None:
    near_root = tmp_path / "near-store"
    with pytest.raises(Exception):
        write_store(
            tmp_path / "expired-store",
            OpenAICodexOAuthCredential(
                access_token=synthetic_access_token(exp_delta=timedelta(seconds=-1)),
                refresh_token="synthetic-refresh-token",
