...
def ready_client_token_status(
    *,
    usable: bool = True,
    remaining_ms: int = 1_200_000,
) -> ProviderClientTokenStatus:
    return ProviderClientTokenStatus(
        ...
        expires_at_utc=NOW + timedelta(milliseconds=remaining_ms),
        remaining_lifetime_ms=remaining_ms,
        usable_for_bounded_lease=usable,
    )
...
def ready_credential_status(
    *,
    ...
    client_token_status: ProviderClientTokenStatus | None | bool = True,
) -> ProviderCredentialStatus:
    status = (
        ready_client_token_status()
        if client_token_status is True
        else client_token_status
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ready_request(
                credential_status=ready_credential_status(client_token_status=None)
            ),
            {},
            "client_token_metadata_required",
        ),
        (
            ready_request(
                credential_status=ready_credential_status(
                    client_token_status=ready_client_token_status(
                        usable=False, remaining_ms=299_999
                    )
                )
            ),
            {},
...
def test_worker_ready_resolution_rejects_invalid_metadata(
    ...
    assert "auth.json" not in str(exc_info.value).lower()
